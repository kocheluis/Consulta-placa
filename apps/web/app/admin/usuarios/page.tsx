...
import { listCupoUsers } from '@/lib/admin-cupo';
import { Icon } from '@/components/ui/Icon';
import { AdminUsuariosClient } from './AdminUsuariosClient';

export const dynamic = 'force-dynamic';
export const metadata = {
  title: 'Cupos de consulta · Admin · PlacaPe',
...
export default async function AdminUsuariosPage() {
  if (!isSupabaseConfigured) {
    return <Shell><Notice icon="info" title="Supabase no configurado" text="Este panel requiere Supabase (Auth + base de datos) configurado en el entorno." /></Shell>;
  }

  const sb = await createClient();
  const { data: { user } } = await sb.auth.getUser();
  if (!user) redirect('/cuenta?next=/admin/usuarios');
    ...
    return (
      <Shell>
        <Notice
          ...
          text={hasAdmins ? 'Tu cuenta no tiene permisos de administrador.' : 'Aún no hay administradores configurados. Define ADMIN_EMAILS en el entorno con tu correo.'}
        />
      </Shell>
    );
  }

  const { migrated, users } = await listCupoUsers();
  return (
    <Shell>
      <AdminUsuariosClient initial={users} migrated={migrated} />
    </Shell>
  );
}
